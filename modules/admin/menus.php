...
if (Common::isPost()){
	...
	if ($method == 'del' && ! empty ( $menu_id )) {
		...
		if(intval($menu_id) <= 100){
			$info = array('type'=>'error','cont'=>ErrorMessage::CAN_NOT_DELETE_SYSTEM_MENU);
		}else{
			$result = MenuUrl::delMenu ($menu_id);	
			if ($result) {
				SysLog::addLog ( UserSession::getUserName(), 'DELETE', 'MenuUrl' ,$menu_id, json_encode($menu->export()) );
				$info = array('type'=>'success','cont'=>'已将菜单链接删除');
			}else{
